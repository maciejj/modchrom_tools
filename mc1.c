...
int totalbpsthreshold;           // minimum number of basepairs per domain

int ndomain;                       // number of domains: 50-400
...
double bpssingle;      // bps per nm
double bpsscoil;   // bps per nm

double setringlen;       // nm
double rdistk;    // force constant for deviation from set distance
double setchainlen;    // nm
double bdistk;    // force constant for deviation from set distance
...
double branchradius;   // nm
double ringradius;       // nm

double initkT;  // initial temperature
double tfactor;      // reduction of temperature at each step, set to 1 for constant T

double restforcefac;   // multiplier for restraint forces

//double branching0=20;  // branching percentage
//MJ
double branching0;  // number of branching points
double branchingk; //branching force constant

double rgyr0;            // target radius of gyration
double rgyrk;        // force constant for radius of gyration restraint

double maxr;          // maximum size (spherical radius)
double maxrk;       // force constant for maximum size restraint

double minangle0;     // minimum angle for connected cylinders
double maxangle0;    // minimum angle for connected cylinders
double minangle1;      // minimum angle for connected cylinders
double maxangle1;     // minimum angle for connected cylinders
double anglek;         // force constant for angle restraint

double contactmin;     // minimum contact distance
double contactk;       // force constant for contact restraint

int    savefreq;      // how often should we save the PDB?
int    eneoutfreq;     // how often should we writeout energy?

double cutofflen;

int    regenlookupfreq; // how often should we regenerate entire lookup table

int    branchmove;       // carry out branch moves
int    remap;            // remap base pairs
...
// Assigning values to the global values:

	num_threads=8;
	check=0;

	//mbps=816394;                      // number of basepairs
    	mbps=atoi(argv[8]);

	totalbpsthreshold=1000;           // minimum number of basepairs per domain
	ndomain=400;                       // number of domains: 50-400

	rfac=0.879;
	cgbeadsize=5.0;
	rise=0.340;
	cgbpbead=(int)(cgbeadsize/rise+0.5);
	maxcgbp=int(mbps/cgbpbead)*1.1;

	bpssingle=1.0/rise;      // bps per nm
	bpsscoil=2.0/rise/0.879;   // bps per nm

	setringlen=20;       // nm
	rdistk=1.0;    // force constant for deviation from set distance
	setchainlen=50;    // nm
	bdistk=0.05;    // force constant for deviation from set distance
	...
	initkT=0.59*5000;  // initial temperature
	tfactor=0.99999;      // reduction of temperature at each step, set to 1 for constant T

	restforcefac=0.001;   // multiplier for restraint forces

	//double branching0=20;  // branching percentage
	//MJ
	branching0=0.00043*mbps;  // number of branching points
	branchingk=1000000; //branching force constant

	rgyr0=0;            // target radius of gyration
	rgyrk=0.00;        // force constant for radius of gyration restraint

	maxr=2000;          // maximum size (spherical radius)
	maxrk=0.0001;       // force constant for maximum size restraint

	minangle0=90.0;     // minimum angle for connected cylinders
	maxangle0=180.0;    // minimum angle for connected cylinders
	minangle1=0.0;      // minimum angle for connected cylinders
	maxangle1=90.0;     // minimum angle for connected cylinders
	anglek=0.01;         // force constant for angle restraint

	contactmin=2.0;     // minimum contact distance
	contactk=1.0;       // force constant for contact restraint

	savefreq=25000;      // how often should we save the PDB?
	eneoutfreq=2500;     // how often should we writeout energy?

	cutofflen;

	regenlookupfreq=25000; // how often should we regenerate entire lookup table

	branchmove=1;       // carry out branch moves
	remap=0;            // remap base pairs

	deltadisp;

	beadradius=5.0;

	basemapoffset=100000;
	natom=0;

// End orf repeated global variables from begining of the file
